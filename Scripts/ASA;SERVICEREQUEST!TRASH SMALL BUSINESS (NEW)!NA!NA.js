/**
 * The below code is related to the ServiceRequest/Trash Small Business (NEW)/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(addrs[0] != null){
		...
		//filter duplicates
		var dupeList = getFilteredRecords(capList, appTypeString, new Array("Assigned To Dept", "Canceled", "Completed"), 7);
	...
	if(isEmpty(dupeList) == true) {
		...
		var emailTo = lookup("COK_Service_Request_Email_To", "KNOXVILLE/KNOX/PS/ADMIN/SOLIDWST/SMALLBUS/NA");
		var emailSubject = "A new Trash Small Business (NEW) Service Request has been added. " + alternateId;
		var locatedDowntown = AInfo["Is your business located downtown?"];
		var makeDo = AInfo["Can your business make do with 4 32- gallon cans or less?"];
                var makeDo1 = AInfo["Can your business fit all trash to one 95 gallon trash cart weekly with no overflow?"];		var locationInfo = "";
		var contactInfo = "";
		...
		if(isEmpty(locatedDowntown)) {
			locatedDowntown = "<< No information provided by caller. >>";
		}

		if(isEmpty(makeDo)) {
			makeDo = "<< No information provided by caller. >>";
		}

		if(isEmpty(makeDo1)) {
			makeDo1 = "<< No information provided by caller. >>";
		}
...
//		"Can your business make do with 4 32- gallon cans or less? - " + makeDo + "<br>" + "<br>" +

		emailBody = "<html>" + 
		"The City Of Knoxville 311 Call Center has received a call regarding the following:" + "<br>" + "<br>" +
		"Is your business located downtown? - " + locatedDowntown + "<br>" + "<br>" +
		"Can your business fit all trash to one 95 gallon trash cart weekly with no overflow? - " + makeDo1 + "<br>" + "<br>" +
