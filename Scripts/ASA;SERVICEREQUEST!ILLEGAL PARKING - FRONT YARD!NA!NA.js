/**
 * The below code is related to the ServiceRequest/Illegal Parking - Front Yard/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(isEmpty(dupeList) == true) {
		...
		var emailSubject = "A new Illegal Parking - Front Yard Service Request has been added. " + alternateId;
		var natureOfRequest = workDescGet(capId);
		var inFrontYard = AInfo["Is the vehicle parked in the front yard?"];
		var describeDetails = AInfo["Please describe details of situation"];
		...
		if(isEmpty(inFrontYard)) {
			inFrontYard = "<< No information provided by caller. >>";
		}
		if(isEmpty(describeDetails)) {
			describeDetails = "<< No information provided by caller. >>";
		}
		...
		"Is the vehicle parked in the front yard? - " + inFrontYard + "<br>" + "<br>" +
		"Please describe details of situation - " + describeDetails + "<br>" + "<br>" +
