/**
 * The below code is related to the ServiceRequest/BULKY TRASH PICKUP REQUEST/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(addrs[0] != null){
		...
		//filter duplicates
		var dupeList = getFilteredRecords(capList, appTypeString, new Array("Assigned To Dept"), 14);
	...
	if(isEmpty(dupeList) == true) {
		//Start Script PS04 Bulky Trash Pickup Request
		...
		editAppSpecific("Work Order Type", "PS04 Bulky Trash Pickup Request", childId);
		...
		workOrderType = "PS04 Bulky Trash Pickup Request"
		copyParcelGisObjects();serviceArea = getGISInfo("KGIS","Public Service Zones","Zone_");
