...
if (v_ChangeAssignedTo == "Yes") {
    ...
    if (isEmpty(v_AssignToFromField) == false) {
        var v_STAFF = lookup("COK_TE_SIGNAL_STAFF", v_AssignToFromField);
        assignCap(v_STAFF);
        editAppSpecific("Change Assigned To", "No");  
        editAppSpecific("Assigned To", v_STAFF);
        editAppSpecific("Assign To", "");
		...
		var emailSubject = "A Signal Work Order has been assigned to you. " + alternateId;
		...
		emailBody = "<html>" + 
		"You have been assigned the following Signal Work Order:" + "<br>" + "<br>" +
		...
		email(emailTo, "trafficsignals@knoxvilletn.gov", emailSubject, emailBody);
		//End Script 32
