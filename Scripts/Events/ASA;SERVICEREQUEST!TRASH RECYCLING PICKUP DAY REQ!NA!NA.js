/**
 * The below code is related to the ServiceRequest/Trash Recycling Pickup Day Req/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(isEmpty(dupeList) == true) {
		...
		var emailTo = lookup("COK_Service_Request_Email_To", "KNOXVILLE/KNOX/PS/ADMIN/SOLIDWST/PKUPDAY/NA");
		var emailSubject = "A new Trash Recycling Pickup Day Request Service Request has been added. " + alternateId;
		var residentialCommercial = AInfo["Is the concern residential or commercial?"];
		var typeService = AInfo["Which type of service?"];
		var locationInfo = "";
		var contactInfo = "";
		...
		if(isEmpty(typeService)) {
			typeService = "<< No information provided by caller. >>";
		}
		...
		"Which type of service? - " + typeService + "<br>" + "<br>" +
