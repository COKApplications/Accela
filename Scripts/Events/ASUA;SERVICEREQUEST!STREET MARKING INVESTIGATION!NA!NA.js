/** 
* The below code is related to the SERVICEREQUEST/STREET MARKING INVESTIGATION record, with
* an ApplicationStatusUpdateAfter event. 
* ASUA:SERVICEREQUEST/STREET MARKING INVESTIGATION/NA/NA
*/
...
var emailSubject = "A new Street Marking Investigation Service Request has been added. " + alternateId;
var natureOfRequest = workDescGet(capId);

var specMtc = AInfo["Is there a specific maintenance issue with any of the following?"];
var ifOther= AInfo["If Other selected, please provide details"];
...
"If Other selected, please provide details - " + ifOther + "<br>" + "<br>" +
...
if(appStatus == "Assigned To Dept") {		//Start Script 09 - Create Traffic Engineering Investigation Record
		var myArray = [[workDescGet(capId)], 
		               [AInfo["Is there a specific maintenance issue with any of the following?"]],
		               [AInfo["If Other selected, please provide details"]]];
		...
		for(i=0; i<myArray.length; i++) {
		     serviceInfo = serviceInfo + myArray[i,i] + "\r";
                ...
                editAppSpecific("311 Service Request Type:", "Street Marking Investigation", childId);
		editAppSpecific("311 Service Request Info:", serviceInfo, childId);
                childserviceArea = getGISInfo("KGIS", "Public Service Zones", "ZONE_");
                var detailedDescription = "Street Marking Investigation" ;
