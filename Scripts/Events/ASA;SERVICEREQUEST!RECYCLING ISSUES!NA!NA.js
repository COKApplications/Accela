/**
 * The below code is related to the ServiceRequest/Recycling Issuest/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(isEmpty(dupeList) == true) {
		...
		var emailSubject = "A new Recycling Issues Service Request has been added. " + alternateId;
		var natureOfRequest = workDescGet(capId);

		var concernType = AInfo["What type of concern are you reporting?"];
		var pickupRecycleBin = AInfo["Pickup Recycle Bin"];
		var updateName = AInfo["Update Name - Address Info"];
		var binSerialNumber = AInfo["Recycle Bin Serial Number"];
		...
		if(isEmpty(pickupRecycleBin)) {pickupRecycleBin = "<< No information provided by caller. >>";}
		...
		"Pickup Recycle Bin - " + pickupRecycleBin + "<br>" + "<br>" +
		"Update Name - Address Info - " + updateName + "<br>" + "<br>" +
		"Recycle Bin Serial Number - " + binSerialNumber + "<br>" + "<br>" +
