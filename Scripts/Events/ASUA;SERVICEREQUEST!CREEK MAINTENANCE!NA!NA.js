/**
 * The below code is related to the SERVICEREQUEST/CREEK MAINTENANCE/NA/NA record, with
 * an ApplicationStatusUpdateAfter event. 
 * ASUA:SERVICEREQUEST/CREEK MAINTENANCE/NA/NA
 */
...
if(appStatus == "Assigned To Dept") {
	...
	var emailSubject = "A new Creek Maintenance Service Request has been added. " + alternateId;
	var natureofrequest = workDescGet(capId);
	var isovergrowth = AInfo["Is the overgrowth causing flooding?"];
	var propertytype = AInfo["Is this on private or public property?"];
	var locationInfo = "";
	var contactInfo = "";
	...
	if(isEmpty(propertytype)) {
		propertytype = "<< No information provided by caller. >>";
	}
	if(isEmpty(isovergrowth)) {
		isovergrowth = "<< No information provided by caller. >>";
	}
	...
	"Is this on private or public property? - " + propertytype + "<br>" + "<br>" +
	"Is the overgrowth causing flooding? - " + isovergrowth + "<br>" + "<br>" +
	...
	var serviceInfo = "311 Service Request Type - " + "What is the nature of the request - " + natureofrequest + "\r" + 
	 "Is this on private or public property? - " + propertytype + "\r" +
	 "Is the overgrowth causing flooding? - " + isovergrowth + "\r";
	...
	editAppSpecific("Incident Type", "Creek Maintenance",childId);
	//
	//End Script 09
	...
	var emailSubject = "You have been assigned a Stormwater Investigation for a Creek Maintenance Service Request. " + CHILD_ALT_ID;
