/**
 * The below code is related to the ServiceRequest/Safety Inspection Electrical/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(addrs[0] != null){
		...
		//filter duplicates
		var dupeList = getFilteredRecords(capList, appTypeString, new Array("Assigned To Dept"), 6);
	...
	if(isEmpty(dupeList) == true) {
		...
		var emailSubject = "A new Safety Inspection Electrical Service Request has been added. " + alternateId;
		var natureOfRequest = workDescGet(capId);

		var resOrCommercial = AInfo["Is this inspection residential or commercial?"];
		...
		if(isEmpty(resOrCommercial)) {resOrCommercial = "<< No information provided by caller. >>";}
		...
		"Is this inspection residential or commercial? - " + resOrCommercial + "<br>" + "<br>" +
