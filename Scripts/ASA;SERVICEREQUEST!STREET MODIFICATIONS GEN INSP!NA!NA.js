/**
 * The below code is related to the ServiceRequest/Street Modifications Gen Insp/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(isEmpty(dupeList) == true) {
		...
		var emailSubject = "A new Street Modifications General Inspection Service Request has been added. " + alternateId;
		var natureOfRequest = workDescGet(capId);

		var typeImprove = AInfo["Which type of improvement are you requesting?"];
		...
		if(isEmpty(typeImprove)) {typeImprove = "<< No information provided by caller. >>";}
		...
		"Which type of improvement are you requesting? - " + typeImprove + "<br>" + "<br>" +
