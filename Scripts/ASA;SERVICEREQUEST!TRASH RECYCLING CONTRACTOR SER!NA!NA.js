/**
 * The below code is related to the ServiceRequest/Trash Recycling Contractor Ser/NA/NA record, with
...
if(addrsObj.getSuccess() ) {
	...
	if(addrs[0] != null){
		...
		//filter duplicates
		var dupeList = getFilteredRecords(capList, appTypeString, new Array("Assigned To Dept"), 5);
	...
	if(isEmpty(dupeList) == true) {
                 //Changed 12-2017 per RFS 25862 - do not auto close SR
		...
		var emailTo = lookup("COK_Service_Request_Email_To", "KNOXVILLE/KNOX/PS/ADMIN/SOLIDWST/TCONTRCT/NA");
		var emailSubject = "A new Trash Recycling Contractor Service Request has been added. " + alternateId;
		var natureRequest = workDescGet(capId);		var missedPickup = AInfo["Is this a missed pickup?"];
		var residentialCommercial = AInfo["Is the concern residential or commercial?"];
		var typeTrash = AInfo["Which type of trash or recycling concern?"];
		var recurringIssue = AInfo["Is this a recurring issue?"];
		var locationInfo = "";
		var contactInfo = "";
		...
		if(isEmpty(missedPickup)) {
			missedPickup = "<< No information provided by caller. >>";
		}
		...
		if(isEmpty(typeTrash)) {
			typeTrash = "<< No information provided by caller. >>";
		}

		if(isEmpty(recurringIssue)) {
			recurringIssue = "<< No information provided by caller. >>";
		}
		...
		"Is this a missed pickup? - " + missedPickup + "<br>" + "<br>" +
		"Is the concern residential or commercial? - " + residentialCommercial + "<br>" + "<br>" +
		"Which type of trash or recycling concern? - " + typeTrash + "<br>" + "<br>" +
		"Is this a recurring issue? - " + recurringIssue + "<br>" + "<br>" +
