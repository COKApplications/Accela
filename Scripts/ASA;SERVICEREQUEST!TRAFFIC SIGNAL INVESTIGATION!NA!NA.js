/**
 * The below code is related to the ServiceRequest/Traffic Signal Investigation/NA/NA record, with
...
editAppSpecific("Intersection", gisIdForLayer);
		...
		var emailSubject = "A new Traffic Signal - School Flashers Investigation Service Request has been added. " + alternateId + " Priority-" + parentPriority;
		var natureRequest = workDescGet(capId);

		//note - no ASI questions - just a place to record inspection result
...
//			}
		...
		copyOwner(capId, childId);
   		...
   		editAppSpecific("311 Service Request Info", natureRequest, childId);
                childserviceArea = getGISInfo("KGIS", "Public Service Zones", "ZONE_");
                var setDescription = updateWorkDesc("Traffic Signal Investigation",childId);
		...
		editAppSpecific("Source of Call", "311", childId); 
                 assignCapToDept ("KNOXVILLE/KNOX/ENG/TRAFFIC/SIGNAL/NA/NA",childId);
