...
function createChild(grp,typ,stype,cat,desc) // optional parent capId
{
	...
	if (appCreateResult.getSuccess())
		...
		//JR - 2018 - added code to populate application name
		//previously record was added with application name = undefined

		COK_Populate_Child_App_Name(newId);
...
//
	...
	if (appCreateResult.getSuccess())
		...
		COK_Populate_Parent_App_Name(newId);
...
function scheduleInspectionChild(iType,DaysAhead) // optional inspector ID.  This function requires dateAdd function
	{
	// DQ - Added Optional 4th parameter inspTime Valid format is HH12:MIAM or AM (SR5110) 
	// DQ - Added Optional 5th parameter inspComm ex. to call without specifying other options params scheduleInspection("Type",5,null,null,"Schedule Comment");
	var inspectorObj = null;
	...
	var schedRes = aa.inspection.scheduleInspection(childGetByCapType("Enforcement/Codes Enforcement/*/*"), inspectorObj, aa.date.parseDate(dateAdd(null,DaysAhead)), inspTime, iType, inspComm)
...
function updateFeeParent(fcode, fsched, fperiod, fqty, finvoice, pDuplicate, pFeeSeq) {
	// Updates an assessed fee with a new Qty.  If not found, adds it; else if invoiced fee found, adds another with adjusted qty.
	// optional param pDuplicate -if "N", won't add another if invoiced fee exists (SR5085)
	// Script will return fee sequence number if new fee is added otherwise it will return null (SR5112)
	// Optional param pSeqNumber, Will attempt to update the specified Fee Sequence Number or Add new (SR5112)
	...
	comment("The parent ID is " + parentId);
	if (pDuplicate == null || pDuplicate.length == 0)
		pDuplicate = "Y";
	else
		pDuplicate = pDuplicate.toUpperCase();
	...
	if (pFeeSeq == null)
		getFeeResult = aa.finance.getFeeItemByFeeCode(parentId, fcode, fperiod);
	else
		getFeeResult = aa.finance.getFeeItemByPK(parentId, pFeeSeq);
	...
	if (getFeeResult.getSuccess()) {
		...
		for (feeNum in feeList)
			...
			{
				var feeSeq = feeList[feeNum].getFeeSeqNbr();
				var editResult = aa.finance.editFeeItemUnit(parentId, adjustedQty + feeList[feeNum].getFeeUnit(), feeSeq);
				feeUpdated = true;
				if (editResult.getSuccess()) {
					logDebug("Updated Qty on Existing Fee Item: " + fcode + " to Qty: " + fqty);
					if (finvoice == "Y") {
	...
	// Add fee if no fee has been updated OR invoiced fee already exists and duplicates are allowed
	if (!feeUpdated && adjustedQty != 0 && (!invFeeFound || invFeeFound && pDuplicate == "Y"))
		feeSeq = addFee(fcode, fsched, fperiod, adjustedQty, finvoice, parentId);
	else
		feeSeq = null;
	updateFeeItemInvoiceFlagParent(feeSeq, finvoice);
	return feeSeq;
} 


function updateFeeItemInvoiceFlagParent(feeSeq,finvoice)
{
	if(feeSeq == null)
		return;
	if(publicUser && !cap.isCompleteCap())
	{
var parentId = getParent();
		var feeItemScript = aa.finance.getFeeItemByPK(parentId,feeSeq);
		if(feeItemScript.getSuccess)
		{
			var feeItem = feeItemScript.getOutput().getF4FeeItem();
			feeItem.setAutoInvoiceFlag(finvoice);
...
function scheduleInspectDateChild(iType,DateToSched) // optional inspector ID.
// DQ - Added Optional 4th parameter inspTime Valid format is HH12:MIAM or AM (SR5110)
// DQ - Added Optional 5th parameter inspComm
	{
	var inspectorObj = null;
	...
	var schedRes = aa.inspection.scheduleInspection(childGetByCapType("Enforcement/Codes Enforcement/*/*"), inspectorObj, aa.date.parseDate(DateToSched), inspTime, iType, inspComm)
...
function getGrandParent() 
	...
	// returns the capId object of the parent.  Assumes only one parent!
	//
	getCapResult = aa.cap.getProjectParents(getParent(),1);
	if (getCapResult.getSuccess())
		{
		parentArray = getCapResult.getOutput();
		if (parentArray.length)
...
function closeTaskParent(wfstr,wfstat,wfcomment,wfnote) // optional process name
	{
	var useProcess = false;
	var processName = "";
	if (arguments.length == 5) 
	...
	var workflowResult = aa.workflow.getTaskItems(getParent(), wfstr, processName, null, null, null);
 	if (workflowResult.getSuccess())
  	 	var wfObj = workflowResult.getOutput();
  	else
  	  	{ logMessage("**ERROR: Failed to get workflow object: " + s_capResult.getErrorMessage()); return false; }
	...
	for (i in wfObj)
		{
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			if (useProcess)
				aa.workflow.handleDisposition(getParent(),stepnumber,processID,wfstat,dispositionDate, wfnote,wfcomment,systemUserObj ,"Y");
			else
				aa.workflow.handleDisposition(getParent(),stepnumber,wfstat,dispositionDate, wfnote,wfcomment,systemUserObj ,"Y");
...
function closeTaskGrandParent(wfstr,wfstat,wfcomment,wfnote) // optional process name
	{
	var useProcess = false;
	var processName = "";
	if (arguments.length == 5) 
	...
	var workflowResult = aa.workflow.getTaskItems(getGrandParent(), wfstr, processName, null, null, null);
 	if (workflowResult.getSuccess())
  	 	var wfObj = workflowResult.getOutput();
  	else
  	  	{ logMessage("**ERROR: Failed to get workflow object: " + s_capResult.getErrorMessage()); return false; }
	...
	for (i in wfObj)
		{
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			if (useProcess)
				aa.workflow.handleDisposition(getGrandParent(),stepnumber,processID,wfstat,dispositionDate, wfnote,wfcomment,systemUserObj ,"Y");
			else
				aa.workflow.handleDisposition(getGrandParent(),stepnumber,wfstat,dispositionDate, wfnote,wfcomment,systemUserObj ,"Y");
...
function getFilteredRecords(capList,capType,capStatusArr,dtAge){
	
	var oArr = new Array();
	for(var x in capList){
		
		if( appMatch(capType,capList[x]) ){
			var tcapId = capList[x];
			var tcap = aa.cap.getCap(tcapId).getOutput();
			var tcapStatus = tcap.getCapStatus();
			var tfileDateObj = tcap.getFileDate();
			var tcapIDString = tcapId.getCustomID();
			for(var st in capStatusArr){
				//Check Status
				if(capStatusArr[st] == tcapStatus ){
					if(dateDiff(tfileDateObj,sysDate) < dtAge){
						if(capIDString != tcapIDString){
							oArr.push("Record ID: " + tcapIDString + " Status: " + tcapStatus);
						}
					}
					break; //status is only going to match at most once
				}
			}
		}
	}
...
function getRecordsByAddressIdBuffer(addressId,svc,layer,gisParcelAttribute,numDistance)
{
	var distanceType = "feet";
	var retArray = new Array();
	var parcelNumber = null;
	
	//get parcel by address ID Address UID
	var priParcelObj = aa.parcel.getPrimaryParcelByRefAddressID(addressId, "N");
	var priParcel = null;
	var priParcelGis = null;
	var addPointBuff = null;
	var capListArray = new Array();
	
	try{
		if(priParcelObj.getSuccess()){
			priParcel = priParcelObj.getOutput()
			var gisObjResult = aa.gis.getParcelGISObjects(priParcel.getParcelNumber()); 
			logDebug("getRecordsByAddressIdBuffer: Parcel Found " + priParcel.getParcelNumber());

			if (gisObjResult.getSuccess()){
				//get the surrounding parcels based upon the buffer
				priParcelGis = gisObjResult.getOutput()[0];
				var parcelObjs = getGISBufferInfoByGISObject(priParcelGis,svc,layer,numDistance,gisParcelAttribute);
				for(x in parcelObjs){
					if(parcelObjs[x][gisParcelAttribute]+"" != ""){
						//get the records associated to the parcels
						var capParObj = aa.cap.getCapListByParcelID(parcelObjs[x][gisParcelAttribute],null);
						if(capParObj.getSuccess()){
							var capParList = capParObj.getOutput();
							for(var c in capParList){
								capListArray[capParList[c].getCustomID()] = capParList[c].getCapID();
							}
						}
					}
				}
		...
		logDebug("getRecordsByAddressIdBuffer: " + err.message);
	}
		
	return capListArray
}

function getGISBufferInfoByGISObject(gisObj,svc,layer,numDistance)
{
	// returns an array of associative arrays
	// each additional parameter will return another value in the array
	//x = getGISBufferInfoByGISObject(gisObject, "flagstaff","Parcels","50","PARCEL_ID1","MAP","BOOK","PARCEL","LOT_AREA");
	//
	//for (x1 in x)
	//   {
	//   aa.print("Object " + x1)
	...
	var bufchk = aa.gis.getBufferByRadius(gisObj, numDistance, distanceType, buf);
...
function copyParcelGisObjectsChild() 
	{
	var capParcelResult = aa.parcel.getParcelandAttribute(childGetByCapType("AMS/Work Order/Public Service/NA"),null);
	if (capParcelResult.getSuccess())
		{
		var Parcels = capParcelResult.getOutput().toArray();
		for (zz in Parcels)
			...
			for (a1 in fGisObj) // for each GIS object on the Cap
				...
				var gisTypeScriptModel = fGisObj[a1];
					...
					var retval = aa.gis.addCapGISObject(childGetByCapType("AMS/Work Order/Public Service/NA"),gisObjModel.getServiceID(),gisObjModel.getLayerId(),gisObjModel.getGisId());
...
/**
 * Copies GIS Objects from Parent CAP to Child CAP
 */
function copyParentGisObjects() 
{
var capParentResult = getParent();
logDebug("capParentResult: " + capParentResult);
if(capParentResult)
	{
		var gisObjResult = aa.gis.getCapGISObjects(capParentResult); // get gis objects on the cap
		if (gisObjResult.getSuccess()) 	
			var fGisObj = gisObjResult.getOutput();
		else
			{ logDebug("**WARNING: Getting GIS objects for Parcel.  Reason is: " + gisObjResult.getErrorType() + ":" + gisObjResult.getErrorMessage()) ; return false }
 ...
 * Copies GIS Objects from Parent CAP to Child CAP
 */
function copyGisObjectsToChild(childCapId) 
{
	logDebug("Inside copyGisObjectsToChild - childCapId: " + childCapId);
	var gisObjResult = aa.gis.getCapGISObjects(capId); // get gis objects on the cap
	logDebug("After var line");
	if (gisObjResult.getSuccess()) 	
		var fGisObj = gisObjResult.getOutput();
	else
		{ logDebug("**WARNING: Getting GIS objects for CAP.  Reason is: " + gisObjResult.getErrorType() + ":" + gisObjResult.getErrorMessage()) ; return false }
	logDebug("fGisObj: " + fGisObj);
	for (a1 in fGisObj) // for each GIS object on the Cap
		{
		var gisTypeScriptModel = fGisObj[a1];
                        var gisObjArray = gisTypeScriptModel.getGISObjects()
			...
			var retval = aa.gis.addCapGISObject(childCapId,gisObjModel.getServiceID(),gisObjModel.getLayerId(),gisObjModel.getGisId());
