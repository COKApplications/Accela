/**
 * The below code is related to the SERVICEREQUEST/Parks Facility Reservation Req/NA/NA record, with
...
editAppSpecific("Council District", CouncilDistrict);
		...
		var emailTo = lookup("COK_Service_Request_Email_To", "KNOXVILLE/KNOX/PARK/NA/NA/NA/NA");
		var emailSubject = "A new Parks Facility Reservation Service Request has been added. " + alternateId;
		var natureOfRequest = workDescGet(capId);
		var facType = AInfo["Which park or facility do you want?"];
		var facDate = AInfo["Which date are you requesting?"];
		var facBegin = AInfo["Beginning Time Slot"];
		var facEnd = AInfo["Ending Time Slot"];
		var facPeople = AInfo["Approximately, how many people are in your party?"];
		var facEventType = AInfo["What type of event?"];
		var locationInfo = "";
		var contactInfo = "";
		...
		if(isEmpty(facDate)) {
			facDate = "<< No information provided by caller. >>";
		}
		if(isEmpty(facBegin)) {
			facBegin = "<< No information provided by caller. >>";
		}
		if(isEmpty(facEnd)) {
			facEnd = "<< No information provided by caller. >>";
		}
		if(isEmpty(facPeople)) {
			facPeople = "<< No information provided by caller. >>";
		}
		if(isEmpty(facEventType)) {
			facEventType = "<< No information provided by caller. >>";
		}
		...
		"Which park or facility do you want? - " + facType + "<br>" + "<br>" +
		"Which date are you requesting? - " + facDate + "<br>" + "<br>" +
		"Beginning Time Slot - " + facBegin + "<br>" + "<br>" +
		"Ending Time Slot - " + facEnd + "<br>" + "<br>" +
		"Approximately, how many people are in your party? - " + facPeople + "<br>" + "<br>" +
		"What type of event? - " + facEventType + "<br>" + "<br>" +
