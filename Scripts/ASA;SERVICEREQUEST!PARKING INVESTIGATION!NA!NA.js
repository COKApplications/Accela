/**
 * The below code is related to the SERVICEREQUEST/Parking Investigation/NA/NA record, with
 * an ApplicationSubmitAfter event. 
 */
showMessage = true;
comment("Running ASA:SERVICEREQUEST/PARKING INVESTIGATION/NA/NA script");
...
//2017-06 - moved create work order section above send email section
//want to include WO number in email
...
//2017-06 - remove duplicate check per RFS 24615 - comment out code
...
//		//filter duplicates
//		var dupeList = getFilteredRecords(capList, appTypeString, new Array("Assigned To Dept"), 20);
//		
...
//if(isEmpty(dupeList) == true) {
		...
		//2017-06 - added code for ASI questions and reported date in work order service order description field

		var childId = createChild("AMS","Work Order","Traffic Engineering","Parking System");
		var parentPriority = capDetail.getPriority();
		var serviceInfo = workDescGet(capId) + " Service Request reported: " + reportedDateString;
...
//	if(isEmpty(dupeList) == true) {
     ...
     natureOfRequest = "Meter " + assetName + " " + "\r" + natureOfRequest; 
		...
		//2017-06 - added code to put ASI questions and reported date in email body
		...
		contactInfo  + "<br>" + "<br>" + 
		"City of Knoxville Service Request Number - " + alternateId + "<br>" +
		"City of Knoxville WO Number - " + childalternateId + 
		"</html>";
