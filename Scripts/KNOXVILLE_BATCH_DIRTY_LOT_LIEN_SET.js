...
| Version 2.0 - Add filing copy of single report in documents for 
|       each record in the set.  08/01/2018.  KCR
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
{	
	...
	for (curRecord in resultObjArray)
		...
		var reportResult = reportAttachToRecord("Lot Lien", "ID", customID);
		logDebug("=====Report results for " + customID + ": " + reportResult);
		//END 16ACC-115422
		
		var capStatus = cap.getCapStatus();
        logDebug("capStatus = " + capStatus);
		if (capStatus == "Bill Owed") {
			//editTaskSpecific("Letter Notice","Date Letter Mailed",dateAdd(null,0));
			closeTask("Bill","Lien In Process","","");
			activateTask("Lien");
 
			var Balance = getUnpaidFeeAmountLiens(capId,"");

			if(Balance == "0") {
				addFee("CE_LATE", "CE_CODE", "FINAL", feeAmountAll(capId,"NEW"), "N");
	} 		else {
				addFee("CE_LATE", "CE_CODE", "FINAL", Balance, "N");
	}

			addFee("CE_FILING", "CE_CODE", "FINAL", 1, "N");

			updateAppStatus("Lien Owed","");
		
			//var inspectionDays = getAppSpecific("Re-Inspection Days");
			
			//if(isEmpty(inspectionDays) == true) {
			//	scheduleInspectDate("LOT Re-Inspection", nextWorkDay(dateAdd(null, 13)), getInspector("LOT Initial Inspection"), "", "Scheduled by Script.");
			//} else if(isEmpty(inspectionDays) == false) {
			//	scheduleInspectDate("LOT Re-Inspection", nextWorkDay(dateAdd(null, inspectionDays - 1)), getInspector("LOT Initial Inspection"), "", "Scheduled by Script.");
			//}
            }
